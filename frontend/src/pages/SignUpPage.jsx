    ...
    <div data-theme={theme} className='w-full flex h-custom' >
      ...
      <div className='w-full lg:w-1/2 h-custom bg-base-100 flex justify-center items-center flex-col gap-4 relative pb-4'>
        <div className='flex justify-center items-center flex-col'>
          ...
          <h1 className='font-bold text-3xl serif text-primary'>Create Account</h1>
          <p className='text-sm text-primary'>Get started with your free account</p>
        </div>
        ...
        <form className='flex flex-col items-center gap-2 w-full px-5' onSubmit={handleSubmit(onSubmit)} > 
          <div className='w-[90%] sm:w-[80%] md:w-[60%]'>
            <label htmlFor='fullName' className='serif text-base-content'>Full Name</label>
            <div className={`border-1 ${ errors.fullName ? 'border-red-500' : 'border-base-content'} rounded-md flex p-2 gap-2 items-center`}>
              <User />
              <input {...register('fullName', {required: {
                value : true,
                message: "User name is required"
              }})} type="text" id='fullName' placeholder='Your Name' className='outline-none border-0 py-1 flex-1 text-base-content rounded-md' />
            </div>
            <div className='h-[20px]'>
              {
                errors.fullName && <p className='text-red-500 font-bold roboto flex gap-2'> <TriangleAlert /> {errors.fullName.message} </p>
              }
            </div> 
          </div>
